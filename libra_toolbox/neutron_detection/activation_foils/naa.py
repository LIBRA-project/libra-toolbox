...
def get_compass_counts(
    directories, bins=None, count_rate=False, skiprows=1, savefile=None
):
        ...
        for source in directories.keys():
            ...
            for filename in os.listdir(directories[source]):
                if filename.endswith(".CSV"):
                    ...
                    if erg_max > 1e5:
                        print(
                            "time data may be in energy_data column for: \n", filename
                        )
                        time_column = 4
                        energy_column = 5
                        time_data = df[time_column].to_numpy()
            ...
            for ch in counts[source].keys():
                root_filename = glob.glob(os.path.join(directories[source], "*.root"))[
                    0
                ]
                root_file = uproot.open(root_filename)
                counts[source][ch]["live_count_time"] = (
                    root_file[f"LiveTime_{ch}"].members["fMilliSec"] / 1000
                )
                counts[source][ch]["real_count_time"] = (
                    root_file[f"RealTime_{ch}"].members["fMilliSec"] / 1000
                )
            ...
            for ch in time_values[source].keys():
                ...
                energy_values[source][ch] = np.nan_to_num(
                    energy_values[source][ch], nan=0
                )
                ...
                if np.abs(total_time - counts[source][ch]["real_count_time"]) > 0.1:
                    ...
                    raise Exception(
                        f"Total time different from root file real count time"
                    )
                if count_rate:
        ...
        for ch in counts[sample].keys():
            if counts[sample][ch]["real_count_time"] < b_count_time[ch]:
                ...
                end_ind = np.nanargmin(
                    np.abs(
                        counts[sample][ch]["real_count_time"]
                        - (times[ch] - times[ch][0])
                    )
                )
                ...
                b_hist = b_hist * (
                    counts[sample][ch]["real_count_time"] / b_count_time[ch]
                )
...
def calibrate_counts(counts, decay_lines, peak_inputs=None, plot_calibration=False):
    ...
    for ch in ch_keys:
        ...
        if plot_calibration:
            xs = np.linspace(
                np.min(calibration_channels[ch]), np.max(calibration_channels[ch])
            )
...
def get_singlepeak_area(hist, bins, peak_erg, search_width=300, plot=False):
    ...
    if plot:
        ...
        ax.plot(
            xvals[search_start:search_end],
            gauss1(xvals[search_start:search_end], *parameters),
        )
...
def get_multipeak_area(hist, bins, peak_ergs, search_width=600, plot=False):
    ...
    if plot:
        ...
        for i in range(len(peak_ergs)):
            ...
            trap_cutoff_ys = (
                parameters[0] + parameters[1] * xvals[peak_starts[i] : peak_ends[i]]
            )
            print("Plot gauss params: {}".format(all_peak_params[i]))
            ax.fill_between(
                xvals[peak_starts[i] : peak_ends[i]],
                gauss(xvals[peak_starts[i] : peak_ends[i]], *all_peak_params[i]),
                trap_cutoff_ys,
                color=colors[i],
                alpha=0.5,
            )
...
def get_peak_areas(
    hist, bins, peak_ergs, overlap_width=200, search_width=400, plot=False
):
    ...
    for erg_group in erg_groups:
        areas += get_multipeak_area(
            hist, bins, erg_group, search_width=len(erg_group) * search_width, plot=plot
        )
        print(areas)
    return areas
...
def energy_efficiency(
    ...
    if plot_eff:
        ...
        for i, ch in enumerate(effs.keys()):
            ...
            ax_eff[i].errorbar(
                energies[ch], effs[ch] * 100, eff_errs[ch] * 100, fmt=".", capsize=3
            )
